...
class server_manager():
            ...
            if web_server_data is not None:
                ...
                if 'plot_request' in data.keys():
                    if len(self.free_ports) > 0:
                        port = self.free_ports[0]
                        t1 = threading.Thread(target=self.respond_to_plot_request, args=(data, port,))
                        t1.start()          
                    else:
    ...
    def respond_to_plot_request(self, data, port):
        self.launch_servers([port]) 
        time.sleep(1.5)
        self.r.publish(data['plot_request'], {'port':port})
